...
pdf_folder = "/home/ninemax/work/bmy/pdf_api_server_原版_7-25/pdf_2.0_test/测试pdf"
MIN_PAGES = 100
...
if __name__ == "__main__":
    ...
    # 优先挑选超长PDF：统计页数，保留大于MIN_PAGES的pdf
    long_pdfs = []
    print(f"正在筛选页数大于{MIN_PAGES}页的超长论文PDF，请稍候......")
    for file in tqdm(pdf_files, desc="统计页数"):
        pages = get_pdf_pages(file)
        if pages is not None and pages >= MIN_PAGES:
            long_pdfs.append((file, pages))
    long_pdfs.sort(key=lambda x: -x[1])  # 按页数从多到少排序

    if not long_pdfs:
        print(f"在目录 {pdf_folder} 下没有页数超过 {MIN_PAGES} 页的PDF!")
        exit(1)
    else:
        print(f"共筛选出{len(long_pdfs)}个超长PDF用于并行解析（最大页数：{long_pdfs[0][1]}）")

    # 只测最长的N个大论文
    N = min(len(long_pdfs), 2)
    tasks = [(file_path, f"longpdf_{i}_pages{pages}") for i, (file_path, pages) in enumerate(long_pdfs[:N])]
    results = []
    pool_size = 2
